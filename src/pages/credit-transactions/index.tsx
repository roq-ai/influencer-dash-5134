...
type ColumnType = ColumnDef<CreditTransactionInterface, unknown>;

interface CreditTransactionListPageProps {
...
export function CreditTransactionListPage(props: CreditTransactionListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CreditTransactionInterface>>(
    () => `/credit-transactions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CreditTransactionInterface) => {
    if (hasAccess('credit_transaction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/credit-transactions/view/${row.id}`);
    }
  };
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="credit_transaction" />
